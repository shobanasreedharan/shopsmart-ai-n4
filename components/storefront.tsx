...
import {
  ...
  PartyPopper, Flame, Tv, Loader2, ExternalLink, ShoppingCart,
} from "lucide-react"
...
const TABS: {
...
}[] = [
  {
    ...
    placeholder: "e.g. Kids birthday party with superhero theme",
...
function AmazonCard({ product, sectionTitle }: { product: AmazonProduct; sectionTitle: string }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-[#e0ddd5] p-4 flex flex-col gap-3 hover:border-[#6dcfa0] hover:shadow-sm transition-all">
      {/* Image area */}
      <div className="bg-[#f9f7f3] rounded-lg h-36 flex items-center justify-center overflow-hidden">
...
export function Storefront({
  ...
  useEffect(() => {
    ...
    const planId = new URLSearchParams(window.location.search).get("plan") || localStorage.getItem("lastPlanId")
    if (!planId) return
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/generate-plan", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ description, type: activeTab, guestCount: col1Value, budget: col2Value }),
      })
      if (!res.ok) {
    ...
    <div className="min-h-screen bg-[#f5f2ec]">
      ...
      {preview && (
        <div className="flex items-center justify-center gap-2 bg-amber-50 border-b border-amber-200 px-4 py-2 text-xs text-amber-800">
          ...
          <span>Preview catalog. Connect &amp; seed DynamoDB (POST <code className="rounded bg-amber-100 px-1 py-0.5">/api/seed</code>) for live catalog.</span>
        </div>
      )}
      ...
      <nav className="sticky top-0 z-30 bg-[#111] h-16 flex items-center justify-between px-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[#888] text-[10px] tracking-widest uppercase mt-0.5">Event Intelligence</div>
          </div>
        </div>
        <button
      ...
      <main className="mx-auto max-w-3xl px-4 pb-16 pt-10">
        ...
        <div className="bg-white rounded-2xl border border-[#e0ddd5] px-10 py-9">
            ...
            {TABS.map((t) => (
              <button
                ...
                className={cn(
                  ...
                  activeTab === t.id ? "bg-[#111] text-white border-[#111]" : "bg-[#f5f2ec] text-[#555] border-[#ddd] hover:bg-[#e8e4dc]",
                )}
              >
                {t.icon} {t.label}
              </button>
          ...
          <div className="grid grid-cols-2 gap-4 mt-5">
            <div>
              <label className="block text-[11px] tracking-widest uppercase text-[#999] font-medium mb-2">{tab.col1Label}</label>
              <input
                type="text"
                value={col1Value}
            ...
            <div>
              <label className="block text-[11px] tracking-widest uppercase text-[#999] font-medium mb-2">Budget</label>
              <select
                value={col2Value}
          ...
          <button
            ...
            className={cn(
              ...
              generating || !description.trim() ? "bg-[#555] text-[#999] cursor-not-allowed" : "bg-[#111] text-white hover:bg-[#222]",
            )}
          >
            {generating ? <><Loader2 className="size-4 animate-spin" /> Finding your supplies…</> : <>✦ {tab.cta}</>}
          </button>

          {generateError && <p className="text-xs text-red-500 mt-3 text-center">{generateError}</p>}
          ...
          <div className="mt-6 bg-white rounded-2xl border border-[#e0ddd5] px-6 py-5">
            ...
            <div className="flex flex-wrap gap-2">
              {EVENT_CHIPS.map((chip) => (
                <button
                  key={chip}
                  onClick={() => setDescription(chip.replace(/[🎉🔧]/g, "").trim())}
          ...
          <section className="mt-8" aria-labelledby="catalog-heading">
            <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h2 id="catalog-heading" className="text-lg font-medium text-[#111] leading-tight">{generatedPlan.title}</h2>
                  <p className="text-xs text-[#888] mt-0.5 flex items-center gap-1.5">
                    ...
                    {planSource === "cache" && <span className="bg-[#f0faf5] text-[#0b6b3a] border border-[#b5e4cc] text-[10px] px-2 py-0.5 rounded-full">✓ From cache</span>}
                    {planSource === "ai" && <span className="bg-[#f0f0ff] text-[#3333aa] border border-[#c0c0f0] text-[10px] px-2 py-0.5 rounded-full">✦ AI generated</span>}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Button onClick={() => setChatOpen(true)} size="sm" variant="outline">Refine with AI</Button>
                <Button onClick={clearPlan} size="sm" variant="ghost"><X className="size-4" /> Clear</Button>
              </div>
            </div>
              ...
              {generatedPlan.sections.map((section, i) => (
                <button
                  ...
                  className={cn(
                    ...
                    activeSection === i ? "bg-[#111] text-white border-[#111]" : "bg-white text-[#555] border-[#ddd] hover:border-[#999]",
                  )}
                >
                  <span>{getCategoryIcon(section.title)}</span>
                  {section.title}
                  <span className={cn("text-xs", activeSection === i ? "text-white/70" : "text-[#aaa]")}>{section.products.length}</span>
                </button>
              ))}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
              {sectionProducts.map((product) => (
                <AmazonCard key={product.asin} product={product} sectionTitle={currentSection?.title ?? ""} />
              ))}
            </div>
          </section>
        )}
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      activePlanId === id ? "bg-[#f0faf5] border-[#6dcfa0]" : "border-[#e0ddd5] hover:border-[#aaa] bg-[#f9f7f3]",
                    )}
                  >
      ...
      <div
        ...
        className={cn("fixed inset-0 z-40 bg-black/40 transition-opacity", chatOpen ? "opacity-100" : "pointer-events-none opacity-0")}
      ...
      >
        <div className="flex items-center justify-between border-b border-[#e0ddd5] px-5 py-4">
          ...
          <button aria-label="Close assistant" onClick={() => setChatOpen(false)} className="text-[#999] hover:text-[#111] transition-colors">
            <X className="size-5" />
          </button>
        </div>
